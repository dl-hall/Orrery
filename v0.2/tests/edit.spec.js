...
test('right-click creates roles and processes; right-drag connects them', async ({ page }) => {
  ...
  d = await data(page);
  expect(d.processes[3].roles.map(l => l.role)).toContain(newRole.id);
  ...
  expect((await data(page)).processes.map(p => p.roles.length)).toEqual([3, 3, 6, 5]);
  ...
  expect(d.processes[4].name).toBe('New process');
});
...
test('process panel: RACI, notes, meetings and documents', async ({ page }) => {
  ...
  let p = (await data(page)).processes[2];
  ...
  expect(d.processes[2].meetings).toContain(made.id);
  ...
  d = await data(page);
  expect(d.processes[2].documents).toContain('doc-product-announcement');
  // Unlink a document.
  await row(page, 'documents', 'doc-technical-feasibility').getByRole('button', { name: /Remove document/ }).click();
  expect((await data(page)).processes[2].documents).not.toContain('doc-technical-feasibility');
  ...
  expect((await data(page)).processes[2].roles.map(l => l.role)).toContain('rol-market-analyst');
  await expect(page.locator('#stage .edges line')).toHaveCount(17);
});
...
test('the rubber band disappears after a right-drag, whether or not it connects', async ({ page }) => {
  ...
  expect((await data(page)).processes[3].roles.some(l => l.role === 'rol-scientist')).toBe(true);
...
test('Add process roles: with two processes a drop-down picks one or all', async ({ page }) => {
  ...
  await page.evaluate(() => {
    ...
    d.processes[2].meetings.push('mtg-sync'); d.processes[3].meetings.push('mtg-sync');
    window.orrery.load(d);
    window.orrery.edit(true);

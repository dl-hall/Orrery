...
test('process table: one column per process with its flows, roles, meetings and documents', async ({ page }) => {
  await openApp(page);
  await page.evaluate(() => window.orrery.setView('process'));
  await expect(page.locator('#stage .edges line')).toHaveCount(0);
  await expect(page.locator('#stage g.card[data-kind="header"]')).toHaveCount(4);
  await expect(page.locator('#stage g.card[data-kind="header"][data-type="process"]')).toHaveCount(4);
  ...
  await expect(inCol(page, 'pro-product-marketing-announcement', '[data-kind="row"]')).toHaveCount(6);
  // Roles appear under every process they belong to.
  await expect(page.locator('#stage g.card[data-id="rol-system-engineer"]')).toHaveCount(3);
  // Order in a column: flows in, roles, meetings, documents, flows out.
  ...
  expect(types).toEqual(['process', 'role', 'role', 'role', 'role', 'role', 'role', 'meeting', 'document', 'document', 'process']);
});
...
test('role table: department super-headers, and no roles under roles', async ({ page }) => {
  ...
  await expect(inCol(page, se, '[data-kind="row"][data-type="process"]')).toHaveCount(3);
  await expect(inCol(page, se, '[data-kind="row"][data-type="meeting"]')).toContainText('CHAIR');
  await expect(inCol(page, se, '[data-kind="row"][data-type="document"]')).toHaveCount(4);
...
test('selection works in table views', async ({ page }) => {
  ...
  await expect(page.locator('#stage g.card.selected')).toHaveCount(3);
});
...
test('tables: hovering a row lights its copies and every column header', async ({ page }) => {
  ...
  expect(await dimmed(page.locator('#stage g.card[data-id="rol-system-engineer"]'))).toEqual([false, false, false]);
  expect(await dimmed(page.locator('#stage g.card[data-kind="header"]'))).toEqual([false, false, false, false]);

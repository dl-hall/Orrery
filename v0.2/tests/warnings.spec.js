...
test('the example has one warning: the two roadmaps feed each other with no role in common', async ({ page }) => {
  await openApp(page);
  expect(await rules(page)).toEqual([['flow-no-shared-role', 'pro-product-roadmap', null]]);
  await expect(page.locator('#btn-warnings .count')).toHaveText('1');
});
...
test('each rule fires, and reviewers are exempt', async ({ page }) => {
  ...
  expect(got).toEqual(expect.arrayContaining([
    ...
    ['flow-no-shared-role', 'pro-product-roadmap', null],
  ]));
  ...
  await expect(page.locator('#btn-warnings .count')).toHaveText('7');
});
...
test('the list opens from the panel and with W, and an entry selects its item', async ({ page }) => {
  ...
  await expect(dlg.locator('.w-group')).toHaveCount(7);
  await expect(dlg.locator('.w-item')).toHaveCount(7);

...
test('example renders 8 roles, 2 processes and 10 links', async ({ page }) => {
  await openApp(page);
  await expect(page.locator('#stage g.card[data-type="role"][data-kind="node"]')).toHaveCount(8);
  await expect(page.locator('#stage g.card[data-type="process"][data-kind="node"]')).toHaveCount(2);
...
test('an empty file opens as an empty organisation', async ({ page }) => {
  ...
  expect(d).toMatchObject({ orrery: '0.1', roles: [], processes: [], meetings: [], documents: [], departments: [], layout: {} });
});
